(function (global, factory) {
    ...
    function layout(element) {
      ...
      containers.detailsClear = containers.detailsContainer.append('button').classed('wc-component__details-clear wc-hidden', true).text('Clear'); // content
    ...
    function styles() {
      ...
      \----------------------------------------------------------------------------*****/
      '.wc-component--kdigo {', '}', '.wc-subcomponent--kdigo-scatter-plot {', '    position: relative;', '}', '.wc-subcomponent--kdigo-scatter-plot circle.wc-data-mark {', '    cursor: pointer;', '    stroke: black;', '    fill-opacity: 1;', '    stroke-width: 1;', '}', '.wc-subcomponent--kdigo-scatter-plot circle.wc-data-mark:hover {', '    stroke-width: 3;', '}', '.wc-subcomponent--kdigo-scatter-plot circle.wc-data-mark.wc-selected {', '    stroke-width: 3;', '}', '.kdigo-stage {', '    stroke: black;', '}', '.wc-subcomponent--kdigo-scatter-plot .tick line {', '    stroke-opacity: .4;', '}', '.wc-subcomponent--kdigo-scatter-plot .legend--kdigo {', '    height: 20px;', '    margin: 8px 0px;', '}', '.wc-subcomponent__legend {', '    position: absolute;', //'    top: 14;',
      //'    bottom: 0;',
      ...
      /*****----------------------------------------------------------------------------\
        component: details
      \----------------------------------------------------------------------------*****/
      '.wc-component__details-clear {', '    float: right;', '    margin: 0.5em;', '}', '.wc-subcomponent__details-participant {', '    list-style: none;', '    padding: 0;', '    width: 100%;', '}', '.wc-details__li {', '    display: inline-block;', '    text-align: center;', '    padding: 0.5em;', '}', '.wc-details__label {', '    font-size: 0.8em;', '}', '.wc-details__value {', '}',
      /*****----------------------------------------------------------------------------\
    ...
    function drawVisitPath() {
      var _this = this;

      var chart = this;
      var config = chart.config;
      var data = this.nepExplorer.data.participant;
      var x_data = data.values.find(function (d) {
        return d.key === _this.config.x.measure;
      }).values.data;
      var x_studyDays = x_data.map(function (d) {
        return d.studyday;
      });
      var y_data = data.values.find(function (d) {
      ...
      var byStudyDay = studyDays.map(function (studyDay) {
        var visitObj = {};
        visitObj.studyday = +studyDay;
        visitObj.visit = config.visit_col ? x_data.find(function (d) {
          return d.studyday === studyDay;
        })[config.visit_col] : null;
        visitObj.visitn = config.visitn_col ? x_data.find(function (d) {
          return d.studyday === studyDay;
        })[config.visitn_col] : null;
        if (config.color_by) visitObj[config.color_by] = x_data[0][config.color_by]; // Get x coordinate.

        visitObj.xMatch = x_data.find(function (d) {
          return d.studyday === studyDay;
        });
      ...
      var myLine = d3.svg.line().x(function (d) {
        return chart.x(d.xClamped);
      }).y(function (d) {
        return chart.y(d.yClamped);
      });
      chart.visitPath.moveToFront();
      var path = chart.visitPath.append('path').attr('class', 'participant-visits').datum(byStudyDay).attr('d', myLine).attr('stroke', function (d) {
        return chart.colorScale(data[config.color_by]);
      }).attr('stroke-width', '2px').attr('fill', 'none'); //Little trick for animating line drawing

      var totalLength = path.node().getTotalLength();
      path.attr('stroke-dasharray', totalLength + ' ' + totalLength).attr('stroke-dashoffset', totalLength).transition().duration(2000).ease('linear').attr('stroke-dashoffset', 0); //draw visit points

      var visitPoints = chart.visitPath.selectAll('g.visit-point').data(byStudyDay).enter().append('g').attr('class', 'visit-point');
      ...
      .transition().delay(2000).duration(200).attr('r', 4); //custom titles for points on mouseover
    ...
    function addClearFunctionality() {
      ...
      this.containers.detailsClear.classed('wc-hidden', false).on('click', function () {
        ...
        _this.kdigoScatterPlot.marks.find(function (mark) {
    ...
    function displayParticipantDetails() {
      ...
      lis.append('div').classed('wc-details__value', true).text(function (d) {
        return _this.data.participant[d.value_col];
      });
    }
    ...
    function drawTimeSeriesCharts() {
      ...
      var _loop = function _loop(name) {
        ...
        var chartData = d3.merge(_this.data.participant.values.filter(function (d) {
    ...
    function init(data, participant) {
      ...
      addVariables(this);
      this.data.participants = defineParticipantLevelData(this); // Initialize KDIGO scatter plot.
      ...
      if (participant) {
        ...
        this.data.participant = this.data.participants.find(function (d) {
          return d.key === participant;
        });
        this.kdigoScatterPlot.marks.find(function (mark) {
          return mark.type === 'circle';
    ...
    function addPointClick() {
      ...
      points.on('mouseover', function (d) {
        ...
        chart.nepExplorer.data.participant = chart.nepExplorer.data.participants.find(function (d) {
          return d.key === chart.nepExplorer.participant;
        }); // Update points.

        points.classed('wc-selected', false);
        d3$1.select(this).classed('wc-selected', true);
        drawVisitPath.call(chart); // Update participant details section
