(function (global, factory) {
    ...
    function layout(element) {
      ...
      containers.leftColumn = containers.main.append('div').classed('wc-column wc-column--left', true); // section: controls

      containers.controls = containers.leftColumn.append('div').classed('wc-section wc-section--controls', true); // column: right

      containers.rightColumn = containers.main.append('div').classed('wc-column wc-column--right', true); // section: population

      containers.population = containers.rightColumn.append('div').classed('wc-section wc-section--population', true); // component: kdigo scatter plot

      containers.kdigo = containers.population.append('div').classed('wc-component wc-component--kdigo', true); // header

      containers.kdigoHeader = containers.kdigo.append('div').classed('wc-header wc-header--kdigo-scatter-plot', true).text('KDIGO Scatter Plot'); // content

      containers.kdigoScatterPlot = containers.kdigo.append('div').classed('wc-subcomponent wc-subcomponent--kdigo-scatter-plot', true);
      containers.kdigoLegend = containers.kdigoScatterPlot.append('div').classed('wc-subcomponent__legend', true); // section: participant

      containers.participant = containers.rightColumn.append('div').classed('wc-section wc-section--participant', true); // component: details

      containers.detailsContainer = containers.participant.append('div').classed('wc-component wc-component--details-container', true); // header

      containers.detailsHeader = containers.detailsContainer.append('div').classed('wc-header wc-header--details', true);
      containers.detailsHeaderText = containers.detailsHeader.append('span').classed('wc-header__text', true).text('Click a point to view participant details.');
      containers.detailsClear = containers.detailsHeader.append('button').classed('wc-header__button wc-component__details-clear wc-hidden', true).text('Clear'); // content

      containers.detailsParticipant = containers.detailsContainer.append('ul').classed('wc-subcomponent wc-subcomponent__details-participant wc-hidden', true); // component: time series
    ...
    function styles() {
      var styles = ['.wc-framework {', '    width: 100%;', '    display: inline-block;', '}', '.wc-hidden {', '    display: none !important;', '}', '.wc-invisible {', '    visibility: hidden;', '}', '.wc-column {', '    display: inline-block;', '}', '.wc-section {', '    display: inline-block;', '    width: 100%;', '}', '.wc-component {', '    display: inline-block;', '    width: 100%;', '}', '.wc-header {', '    border-top: 2px solid black;', '    border-bottom: 2px solid black;', '    padding: 0.2em;', '    font-weight: bold;', '}', '.wc-subcomponent {', '    width: 50%;', '    min-width: 500px;', '}', '.wc-chart {', '    width: 80%;', '    min-width: 400px;', '    display: inline-block;', '}',
      /***--------------------------------------------------------------------------------------\
      column: left
      \--------------------------------------------------------------------------------------***/
      ...
      /****---------------------------------------------------------------------------------\
        section: controls
      \---------------------------------------------------------------------------------****/
      '.wc-section--controls {', '}', '.control-group {', '    width: 100%;', '}', '.changer {', '    width: 200px !important;', '}', '.wc-subheader {', '    border-top: 1px solid black;', '    border-bottom: 1px solid black;', '    margin-right: 1em;', '    margin-bottom: 1em;', '}', '.wc-subheader__text {', '    display: block;', '}', '.wc-subheader__content {', '    font-size: .8em;', '}',
      /***--------------------------------------------------------------------------------------\
      column: right
      \--------------------------------------------------------------------------------------***/
      '.wc-column--right {', '    width: 78%;', '    float: right;', '}',
      /****---------------------------------------------------------------------------------\
        section: population
      \---------------------------------------------------------------------------------****/
      ...
      \----------------------------------------------------------------------------*****/
      '.wc-component--kdigo {', '}', '.wc-subcomponent--kdigo-scatter-plot {', '    position: relative;', '}', '.wc-subcomponent--kdigo-scatter-plot circle.wc-hover-mark,', '.wc-subcomponent--kdigo-scatter-plot circle.wc-data-mark {', '    cursor: pointer;', '}', '.wc-subcomponent--kdigo-scatter-plot .wc-hysteresis-point-container circle.wc-hover-mark,', '.wc-subcomponent--kdigo-scatter-plot .wc-hysteresis-point-container circle.wc-data-mark {', '    cursor: default;', '}', '.wc-subcomponent--kdigo-scatter-plot circle.wc-data-mark {', //'    stroke: black;',
      //'    fill-opacity: 1;',
      '    stroke-width: 1;', '}', 'circle.wc-data-mark.wc-highlighted,', 'circle.wc-hysteresis-point.wc-highlighted,', 'circle.wc-data-mark.wc-selected {', '    stroke-width: 3;', '    stroke: black;', '}', 'circle.wc-data-mark.wc-deemphasized:not(.wc-highlighted) {', '    stroke-opacity: .25;', '    fill-opacity: .25;', '}', '.kdigo-stage {', '    stroke: black;', '}', '.wc-subcomponent--kdigo-scatter-plot .tick line {', '    stroke-opacity: .4;', '}', '.wc-subcomponent--kdigo-scatter-plot .legend--kdigo {', '    height: 20px;', '    margin: 8px 0px;', '}', '.wc-subcomponent__legend {', '    position: absolute;', //'    top: 14;',
      //'    bottom: 0;',
      '    left: 80%;', '    width: 100%;', '}', '.wc-subcomponent__legend .wc-chart {', '    width: 100%;', '}', '.wc-subcomponent__legend .interactivity {', '    display: none;', '}', '.wc-subcomponent__legend table {', '    border-collapse: collapse;', '}', '.wc-subcomponent__legend thead {', '    border-top: 2px solid #999;', '    border-bottom: 2px solid #999;', '}', '.wc-subcomponent__legend thead tr {', '    padding: 0.1em;', '}', '.wc-subcomponent__legend tbody tr:last-child {', '    border-top: 2px solid #999;', '}', '.wc-subcomponent__legend tbody td {', '    text-align: center;', '    font-size: 0.9em;', '    padding: 0 0.5em 0 0.5em;', '}',
      /****---------------------------------------------------------------------------------\
        section: participant
      \---------------------------------------------------------------------------------****/
      ...
      /*****----------------------------------------------------------------------------\
        component: details
      \----------------------------------------------------------------------------*****/
      '.wc-component__details-clear {', '    margin: 0.5em;', '}', '.wc-subcomponent__details-participant {', '    list-style: none;', '    padding: 0;', '    width: 100%;', '}', '.wc-details__li {', '    display: inline-block;', '    text-align: center;', '    padding: 0.5em;', '}', '.wc-details__label {', '    font-size: 0.8em;', '}', '.wc-details__value {', '}',
      /*****----------------------------------------------------------------------------\
        component: time series
      \----------------------------------------------------------------------------*****/
      '.wc-diff .wc-hover-line {', '    stroke: #fff;', '    stroke-width: 16;', '    stroke-opacity: 0;', '}', '.wc-diff .wc-visible-line {', '    stroke-width: 2;', '    stroke: #8E4C6A;', '    stroke-dasharray: 3 3;', '}', '.wc-diff .wc-visible-line.wc-hovered {', '    stroke-dasharray: 0;', '}', '.wc-component--time-series .axis-title {', '    font-size: 14px;', '    font-weight: bold;', '}', '.wc-chart-container {', '    position: relative;', '}', '.legend--time-series {', '    position: absolute;', '    left: 80%;', '    width: 100%;', '}', '.legend--time-series .legend-item {', '    float: left;', '    clear: left;', '    margin-right: 0 !important;', '    height: 14px;', '}', '.legend--time-series .legend-mark-text {', '    display: none;', '}', '.legend--time-series .legend-label {', '    font-size: 12px;', '}', '.legend--time-series .legend-color-block {', '}', '.legend--time-series .legend-mark {', '    transform: translate(0,2px);', '    stroke-width: 4;', '}', '.wc-reference-lines {', '    cursor: help;', '}', '.wc-reference-line {', '    x1: 0;', '    stroke: #999;', '    stroke-dasharray: 3 3;', '}', '.wc-reference-label {', '    text-anchor: end;', '    alignment-baseline: baseline;', '    fill: #999;', '}'];
    ...
    function round(value) {
      ...
      return _float < 1 ? Math.round(_float * 1000) / 1000 : _float < 10 ? Math.round(_float * 100) / 100 : _float < 100 ? Math.round(_float * 10) / 10 : Math.round(_float);
    }
    ...
    function getExtremum(visitWindows, extremum, variable) {
      return visitWindows.length ? d3[extremum](visitWindows, function (visitWindow) {
    ...
    function defineParticipantLevelData(_ref) {
      ...
      var participantLevel = d3.nest().key(function (d) {
        ...
        // visit comparisons
        var studydays = _toConsumableArray(new Set(data.map(function (d) {
          return d.studyday;
        })).values());

        var visitWindows = cartesianProduct(studydays, studydays).filter(function (d) {
          return d[1] > d[0];
        ...
        .map(function (visitWindow) {
          ...
          var pchg = vis1.result > 0 ? round((vis2.result / vis1.result - 1) * 100) : null;
        ...
        data.forEach(function (d) {
          ...
          d.pchg = baseline && baseline.result > 0 ? round((d.result / baseline.result - 1) * 100) : null;
      ...
      var results = ['chg', 'pchg', 'pchg_inv', 'chg_b', 'pchg_b', 'pchg_b_inv'];
      participantLevel.forEach(function (d) {
        ...
        measures.forEach(function (measure) {
          results.forEach(function (result) {
            ...
            d[measure_result] = measure_datum ? measure_datum.values["max_".concat(result)] : null;
          });
        }); // color
    ...
    function clearParticipant() {
      if (this.nepExplorer.participant === undefined || this.nepExplorer.data.participant !== undefined) {
        ...
        containers.detailsParticipant.classed('wc-hidden', true).selectAll('*').remove();
        containers.timeSeries.classed('wc-hidden', true);
        this.svg.selectAll('circle.wc-data-mark').classed('wc-deemphasized', false).classed('wc-selected', false);
    ...
    function merge(value, replacement) {
      ...
      if (_typeof(value) !== _typeof(replacement)) {
        console.warn("Type difference in merge():\nValue: [ ".concat(JSON.stringify(value), " ]\nReplacement: [ ").concat(JSON.stringify(replacement), " ]"));
      ...
      for (var _i = 0, _arr = [].concat(_toConsumableArray(Object.keys(value)), _toConsumableArray(Object.keys(replacement))); _i < _arr.length; _i++) {
      ...
      return {
        ...
        reference_lines: [{
          ...
          tooltip: function tooltip(chart) {
            return chart.filtered_data.filter(function (d) {
              return Array.isArray(chart.config.baseline_value) ? chart.config.baseline_value.includes(d.baseline) : chart.config.baseline_value === d.baseline;
            }).map(function (d) {
      ...
      return {
        ...
        reference_lines: [{
          ...
          tooltip: function tooltip(chart) {
            ...
            }).entries(chart.filtered_data).map(function (d) {
    ...
    function groupControls() {
      ...
      this.controls.controlGroups.each(function (d) {
        var property = d.label.toLowerCase().replace(/[^a-z_0-9]/g, '_');
      ...
      this.controls.filtersHeader = this.controls.wrap.insert('div', '.control-group--subsetter').classed('wc-subheader', true);
      this.controls.filtersHeader.append('span').classed('wc-subheader__text', true).text('Filters');
      this.controls.popCount = this.controls.filtersHeader.append('span').classed('wc-subheader__content', true); // group other controls
      ...
      if (this.controls.settings.size()) {
        this.controls.settingsHeader = this.controls.wrap.insert('div', '.control-group--dropdown').classed('wc-subheader', true);
        this.controls.settingsHeader.append('span').classed('wc-subheader__text', true).text('Settings');
      }
    }
          ...
          switch (d.label) {
            case 'X-axis':
              updateOptionText.call(chart, options, function (text) {
                ...
                var measure = chart.config.measure_values[text.replace("_".concat(result), '')];
                return "".concat(measure, " ").concat(result.replace(/^chg$/, 'Change').replace(/^pchg$/, 'Percent Increase').replace(/^pchg_inv$/, 'Percent Decrease'));
              });
            ...
            case 'Group':
              updateOptionText.call(chart, options, function (text) {
                return text !== 'None' ? chart.config.groups.find(function (group) {
    ...
    function updateVisitWindow() {
      var chart = this;
      this.controls.visitWindow = this.controls.wrap.selectAll('.control-group').filter(function (d) {
    ...
    function addChartContainer(name) {
      var propertyName = name.toLowerCase().split(' ').map(function (str, i) {
        return i === 0 ? str : str.substring(0, 1).toUpperCase() + str.substring(1);
      }).join('');
      var className = name.toLowerCase().split(' ').join('-');
      this.containers[propertyName] = this.svg.append('g').classed("wc-chart-customization wc-chart-customization--".concat(className), true);
    }
    ...
    function setAxisVariables() {//if (this.config.visit_comparison === 'baseline') {
      //    this.config.x.column = this.config.x.column.replace('chg', 'chg_b');
      //    this.config.y.column = this.config.y.column.replace('chg', 'chg_b');
      //}
    }
    ...
    function setAxisLabels() {
      ...
      ['x', 'y'].forEach(function (axis) {
        _this.config[axis].result = _this.config[axis].column.substring(_this.config[axis].column.search(/_.?chg/) + 1);
        _this.config[axis].measure = _this.config.measure_values[_this.config[axis].column.replace("_".concat(_this.config[axis].result), '')];
        _this.config[axis].label = "".concat(_this.config[axis].measure, " ").concat(_this.config[axis].result.replace(/^chg$/, 'Change').replace(/^pchg$/, 'Percent Increase').replace(/^pchg_inv$/, 'Percent Decrease'));
      });
    }

    function identifyCriteria() {
      this.config.criteria = /_chg/.test(this.config.y.column) ? this.config.kdigo_dc_criteria // absolute change
      : this.config.kdigo_criteria; // percent change

      this.nepExplorer.containers.kdigoHeader.text(this.config.criteria === this.config.kdigo_criteria ? 'KDIGO Scatter Plot' : 'KDIGO-DC Scatter Plot');
    }

    function removeNonpositiveRecords() {
      var _this = this;

      this.raw_data = this.config.x.type === 'log' ? this.nepExplorer.data.participants.filter(function (d) {
    ...
    function addVariables$1() {
      ...
      this.nepExplorer.data.participants.forEach(function (participant) {
        // defineKdigoStage
        var kdigo = _this.config.criteria.slice().sort(function (a, b) {
          return b.x - a.x;
        }).find(function (criterion) {
          return criterion.x <= participant[_this.config.x.column] || criterion.y <= participant[_this.config.y.column];
        });

        participant.kdigo = kdigo ? kdigo.label.replace(/stage_(\d)/, 'Stage $1 AKI').replace('no_aki', 'No AKI') : '???'; // identifyMissingParticipants
        ...
        participant.singleVisit = !participant.missingResult && (x !== undefined && x.values.data.filter(function (d) {
          return !isNaN(d.result);
        }).length === 1 || y !== undefined && y.values.data.filter(function (d) {
          return !isNaN(d.result);
        }).length === 1); // results outside visit window

        var xVisitWindow = x !== undefined ? d3.min(x.values.visitWindows, function (visitWindow) {
          return visitWindow.studyday2 - visitWindow.studyday1;
        }) : undefined;
        var yVisitWindow = y !== undefined ? d3.min(y.values.visitWindows, function (visitWindow) {
          return visitWindow.studyday2 - visitWindow.studyday1;
        }) : undefined;
        participant.outsideWindow = !participant.singleVisit && (xVisitWindow > _this.config.visit_window || yVisitWindow > _this.config.visit_window); // log axis and nonpositive maximal change

        participant.nonPositiveChange = !participant.outsideWindow && _this.config.x.type === 'log' && (participant[_this.config.x.column] <= 0 || participant[_this.config.y.column] <= 0);
        participant.status = participant.missingResult ? 'Missing result' : participant.singleVisit ? 'Single visit' : participant.nonPositiveChange ? 'Nonpositive change' : participant.outsideWindow ? 'Outside visit window' : participant.kdigo;
      });
    }
    ...
    function updateGrouping() {
      ...
      if (this.config.color_by && this.config.color_by !== 'None') {
        this.config.color_dom = _toConsumableArray(new Set(this.nepExplorer.data.participants.map(function (participant) {
    ...
    function disableVisitWindowInput() {
    ...
    function detachParticipant() {
      ...
      if (this.nepExplorer.participant !== undefined && !this.filtered_data.find(function (d) {
    ...
    function updatePopCount() {
      ...
      this.filters.forEach(function (filter) {
        _this.nepExplorer.data.filtered = _this.nepExplorer.data.filtered.filter(function (d) {
          return Array.isArray(filter.val) ? filter.val.includes(d[filter.col]) : filter.val === d[filter.col];
        });
      });
      this.controls.popCount.html("<span class = 'numerator'>".concat(this.nepExplorer.data.filtered.length, "</span> of <span class = 'numerator'>").concat(this.nepExplorer.data.participants.length, "</span> participants selected."));
    }

    function updateDomains() {
      this.x_dom[0] = Math.min(this.x_dom[0], this.config.y.type === 'log' ? 1 : 0);
      this.x_dom[1] = Math.max(this.x_dom[1], d3.max(this.config.criteria, function (criterion) {
        return criterion.x;
      }));
      this.y_dom[0] = Math.min(this.y_dom[0], this.config.y.type === 'log' ? 1 : 0);
      this.y_dom[1] = Math.max(this.y_dom[1], d3.max(this.config.criteria, function (criterion) {
        return criterion.y;
      }));
    }
    ...
    function metadata() {
      ...
      var metadata = this.config.criteria.slice().sort(function (a, b) {
        return a.x - b.x;
      }).map(function (d, i) {
        var next = i < _this.config.criteria.length - 1 ? _this.config.criteria[i + 1] : {
          x: _this.x_dom[1],
          y: _this.y_dom[1]
        };
        d.dimensions = [[_this.x_dom[0], next.x], [_this.y_dom[0], next.y]];
        return d;
      }).reverse();
      return metadata;
    }
    ...
    function drawOrigin() {
      ...
      if (this.x_dom[0] < 0) {
        this.containers.origin.append('line').classed('wc-origin wc-origin--x', true).attr({
          x1: this.x(0),
          x2: this.x(0),
          y1: 0,
          y2: this.plot_height,
      ...
      if (this.y_dom[0] < 0) {
        this.containers.origin.append('line').classed('wc-origin wc-origin--y', true).attr({
          x1: 0,
          x2: this.plot_width,
          y1: this.y(0),
          y2: this.y(0),
    ...
    function drawVoronoiDiagram() {
      ...
      var markData = this.nepExplorer.participant && this.config.title === 'KDIGO Scatter Plot' ? mark.data.concat(this.nepExplorer.data.participant.visits.filter(function (d, i) {
        return i !== 0;
      ...
      var voronoiGenerator = d3.geom.voronoi().x(function (d) {
        return _this.x(d.values.x);
      }).y(function (d) {
        return _this.y(d.values.y);
      }).clipExtent([[0, 0], [this.plot_width, this.plot_height]]); // pass coordinates to voronoi generator

      var voronoiData = voronoiGenerator(uniquePoints); // add clipPaths for each voronoi partition

      var voronoiClipPaths = this.containers.voronoiDiagram.append('defs').selectAll('clipPath').data(voronoiData).enter().append('clipPath').attr({
        "class": function _class(d) {
          return 'wc-voronoi__cell';
        },
        id: function id(d) {
          return "wc-voronoi__cell--".concat(d.point.key.toLowerCase().replace(/ /g, '-'));
        }
      }).append('path').attr({
        d: function d(_d) {
      ...
      var hoverMarks = mark.groups.filter(function (d) {
        return _this.x_dom[0] <= d.values.x && d.values.x <= _this.x_dom[1] && _this.y_dom[0] <= d.values.y && d.values.y <= _this.y_dom[1];
      }).append('circle').classed('wc-hover-mark', true).attr('clip-path', function (d) {
        return "url(#wc-voronoi__cell--".concat(d.key.toLowerCase().replace(/ /g, '-'), ")");
        ...
        return _this.y(d.values.y);
      }).attr('r', 50).style('fill', 'lightblue').style('pointer-events', 'all').style('fill-opacity', 0); // add paths for each voronoi partition to view voronoi diagram

      this.svg.selectAll('path.voronoi-partition').remove();

      if (this.nepExplorer.settings.synced.display_voronoi) {
        var voronoiCells = this.containers.voronoiDiagram.selectAll('path.voronoi-partition').data(voronoiData).enter().insert('path', ':first-child').attr({
          "class": function _class(d) {
    ...
    function updateTooltips() {
      ...
      mark.groups.each(function (d, i) {
        ...
        title.text(["Participant ID: ".concat(d.key), "KDIGO: ".concat(participant.kdigo), "".concat(chart.config.x.label, ": ").concat(d.values.x, " (").concat(xVis1.result, " > ").concat(xVis2.result, ")"), "".concat(chart.config.y.label, ": ").concat(d.values.y, " (").concat(yVis1.result, " > ").concat(yVis2.result, ")"), "".concat(chart.config.x.measure, " visit window: ").concat(xVis1.studyday, " > ").concat(xVis2.studyday, " (").concat(xVis1.visit, " > ").concat(xVis2.visit, ")"), "".concat(chart.config.y.measure, " visit window: ").concat(yVis1.studyday, " > ").concat(yVis2.studyday, " (").concat(yVis1.visit, " > ").concat(yVis2.visit, ")")].join('\n'));
      });
    }
    ...
    function addPointHover() {
      this.marks.find(function (mark) {
        return mark.type === 'circle';
      }).groups.selectAll('circle').on('mouseover', function (d) {
      ...
      if (this.containers.hasOwnProperty('hysteresisPlot')) {
        this.containers.hysteresisPlot.selectAll('circle').on('mouseover', function (d) {
          d3.select(this.parentNode).select('.wc-hysteresis-point').classed('wc-highlighted', true);
        }).on('mouseout', function (d) {
          d3.select(this.parentNode).select('.wc-hysteresis-point').classed('wc-highlighted', false);
        });
      }
    }
    ...
    function defineVisitLevelData() {
      ...
      var visits = studydays.map(function (studyday) {
        var visitObj = {
          studyday: studyday
        }; // Get visit value.

        visitObj.visit = _this.config.visit_col ? x_data.find(function (d) {
          return d.studyday === studyday;
        })[_this.config.visit_col] : null;
        visitObj.visitn = _this.config.visitn_col ? x_data.find(function (d) {
          return d.studyday === studyday;
        })[_this.config.visitn_col] : null; // Get x coordinate.
        ...
        if (_this.config.color_by) visitObj[_this.config.color_by] = x_data[0][_this.config.color_by]; // Get KDIGO stage.
        ...
        visitObj.kdigo = kdigo ? kdigo.label.replace(/stage_(\d)/, 'Stage $1 AKI').replace('no_aki', 'No AKI') : '???'; // for voronoi diagram
    ...
    function drawPath() {
      ...
      var visitPath = this.containers.hysteresisPlot.append('path').attr('class', 'participant-visits').datum(this.nepExplorer.data.participant.visits).attr('d', line).attr('stroke', function (d) {
        return _this.colorScale(_this.nepExplorer.data.participant[_this.config.color_by]);
      }).attr('stroke-width', '2px').attr('fill', 'none'); // Animate path.

      var totalLength = visitPath.node().getTotalLength();
      visitPath.attr('stroke-dasharray', totalLength + ' ' + totalLength).attr('stroke-dashoffset', totalLength).transition().duration(2000).ease('linear').attr('stroke-dashoffset', 0);
      return visitPath;
    }
    ...
    function drawPoints() {
      ...
      var chart = this;
      var visitContainers = this.containers.hysteresisPlot.selectAll('g.wc-hysteresis-point-container').data(this.nepExplorer.data.participant.visits).enter().append('g').classed('wc-hysteresis-point-container', true);
      visitContainers.each(function (d) {
        ...
        if (['<', '>'].includes(d.xIsClamped)) visitContainer.append('polygon').classed('wc-hysteresis-point--clamped wc-hysteresis-point--clamped--x', true).attr({
          points: [[x, y], [x, y], [x, y]],
          fill: color
        }).transition().delay(2000).duration(200).attr({
          points: [[x, y - width], [d.xIsClamped === '<' ? x - width : x + width, y], [x, y + width]]
        }); // clamped on the bottom or top of the chart

        if (['<', '>'].includes(d.yIsClamped)) visitContainer.append('polygon').classed('wc-hysteresis-point--clamped wc-hysteresis-point--clamped--y', true).attr({
          points: [[x, y], [x, y], [x, y]],
          fill: color
        }).transition().delay(2000).duration(200).attr({
          points: [[x - width, y], [x + width, y], [x, d.yIsClamped === '<' ? y + width : y - width]]
        }); // not clamped

        if (!d.xIsClamped && !d.yIsClamped) visitContainer.append('circle').classed('wc-hysteresis-point', true).attr({
          cx: x,
          cy: y,
          r: 0,
          fill: color,
        ...
        visitContainer.append('circle').classed('wc-hover-mark', true).attr('clip-path', function (d) {
          return "url(#wc-voronoi__cell--".concat(d.key.toLowerCase().replace(/ /g, '-'), ")");
        }).style('clip-path', function (d) {
          return "url(#wc-voronoi__cell--".concat(d.key.toLowerCase().replace(/ /g, '-'), ")");
        }).attr('cx', x).attr('cy', y).attr('r', 50).style('fill', 'lightblue').style('pointer-events', 'all').style('fill-opacity', 0);
    ...
    function displayParticipantDetails() {
      ...
      nepExplorer.containers.detailsParticipant.classed('wc-hidden', false).selectAll('li').remove();
      var lis = nepExplorer.containers.detailsParticipant.selectAll('li').data(nepExplorer.settings.synced.details).enter().append('li').classed('wc-details__li', true);
    ...
    function drawTimeSeriesCharts() {
      ...
      var _loop = function _loop(name) {
        ...
        var chartData = d3.merge(nepExplorer.data.participant.values.filter(function (d) {
        ...
        if (chartData.length) {
          if (chart.initialized) chart.draw(chartData);else {
    ...
    function addPointClick() {
      ...
      this.marks.find(function (mark) {
        return mark.type === 'circle';
      }).groups.selectAll('circle').on('click', function (d) {
    ...
    function addKdigoLegend() {
      ...
      this.kdigoSummary = this.config.criteria.slice().reverse().map(function (stage) {
        var datum = _objectSpread2({}, stage);

        datum.n = _this.nepExplorer.data.filtered.filter(function (d) {
      ...
      this.statusSummary = d3.nest().key(function (d) {
        return d.status;
      }).rollup(function (data) {
        var datum = {
          label: data[0].status,
        ...
        datum.pct = d3.format('.1%')(datum.rate);
        return datum;
      }).entries(this.nepExplorer.data.filtered).map(function (d) {
      ...
      var summary = [].concat(_toConsumableArray(this.kdigoSummary), _toConsumableArray(this.statusSummary.filter(function (d) {
        return !_this.config.criteria.map(function (stage) {
          return stage.label;
        }).includes(d.label);
      })), [totalRow]);

      if (this.kdigoLegend) {
        this.kdigoLegend.config.headers[0] = this.config.criteria === this.config.kdigo_criteria ? 'KDIGO' : 'KDIGO-DC';
        this.kdigoLegend.draw(summary);
      } else {
        this.kdigoLegend = webcharts.createTable(this.nepExplorer.containers.kdigoLegend.node(), {
          cols: ['label', 'n', 'pct'],
          headers: ['KDIGO', '#', '%'],
          searchable: false,
          sortable: false,
        ...
        this.kdigoLegend.on('draw', function () {
          ...
          this.wrap.selectAll('tbody td').filter(function (d) {
            return d.col === 'label';
          }).style({
    ...
    function setYDomain() {
      ...
      if (this.config.chart === 'uln') this.y_dom[1] = Math.max(this.config.y.domain[1], d3.max(this.filtered_data, function (d) {
        return d[_this.config.y.column];
      }));

      if (this.config.chart === 'bp') {
        this.y_dom[0] = Math.min(this.config.y.domain[0], d3.min(this.filtered_data, function (d) {
          return d[_this.config.y.column];
        }));
        this.y_dom[1] = Math.max(this.config.y.domain[1], d3.max(this.filtered_data, function (d) {
          return d[_this.config.y.column];
        }));
      }

      if (this.config.chart === 'albcreat') this.y_dom[1] = Math.max(this.config.y.domain[1], d3.max(this.filtered_data, function (d) {
        return d[_this.config.y.column];
      }));
    }
    ...
    function drawReferenceLine() {
      ...
      if (this.config.reference_lines) {
        ...
        g.selectAll('line').data(reference_lines).enter().append('line').classed('wc-reference-line', true).attr({
          x2: this.plot_width,
          y1: function y1(d) {
            return _this.y(d.y);
          },
        ...
        }).append('title').classed('wc-reference-tooltip', true).text(function (d) {
          return typeof d.tooltip === 'string' ? d.tooltip : d.tooltip(_this);
        }); // labels

        g.selectAll('text').data(reference_lines).enter().append('text').classed('wc-reference-label', true).attr({
          x: this.plot_width,
          y: function y(d) {
            return _this.y(d.y);
          },
    ...
    function drawDifference() {
      ...
      if (this.config.diff) {
        ...
        var matches = d3.nest().key(function (d) {
          return d.total;
        }).rollup(function (data) {
          var datum = {
            n: data.length
          ...
          return datum;
        }).entries(mark.data).filter(function (d) {
          return d.values.n > 1;
        });
        var diffs = g.selectAll('g').data(matches).enter().append('g').classed('wc-diff', true);
        diffs.append('line').classed('wc-visible-line', true).attr({
          x1: function x1(d) {
        ...
        var hoverLines = diffs.append('line').classed('wc-hover-line', true).attr({
          x1: function x1(d) {
            return _this.x(+d.key);
          },
          x2: function x2(d) {
        ...
        hoverLines.append('title').text(function (d) {
          return "Study Day: ".concat(d.key, "\n").concat(d.values.measure1, ": ").concat(d.values.y1, " (").concat(d.values.result1, ")\n").concat(d.values.measure2, ": ").concat(d.values.y2, " (").concat(d.values.result2, ")\nDifference: ").concat(d.values.diff);
        });
        hoverLines.on('mouseover', function (d) {
          d3.select(this.parentNode).select('.wc-visible-line').classed('wc-hovered', true);
        }).on('mouseout', function (d) {
          d3.select(this.parentNode).select('.wc-visible-line').classed('wc-hovered', false);
        });
      }
    }
    ...
    function updateTooltips$1() {
      ...
      mark.groups.each(function (d, i) {
        ...
        if (datum.baseline !== chart.config.baseline_value && !isNaN(datum.baseline_result) && datum.baseline_result !== null) {
          variables.push({
        ...
        if (datum.xuln !== null) variables.push({
          value_col: 'xuln',
          label: 'Result xULN'
        });
        var tooltip = variables.map(function (variable) {
    ...
    function nepExplorer() {
      ...
      nepExplorer.settings.synced = configuration.syncKdigoScatterPlot(nepExplorer.settings.merged); // controls

      nepExplorer.settings.controls = {
        inputs: configuration.syncControlInputs(configuration.controlInputs(), nepExplorer.settings.synced)
      };
      nepExplorer.controls = webcharts.createControls(nepExplorer.containers.controls.node(), nepExplorer.settings.controls); // chart

      nepExplorer.kdigoScatterPlot = webcharts.createChart(nepExplorer.containers.kdigoScatterPlot.node(), nepExplorer.settings.synced, nepExplorer.controls);
      ...
      Object.keys(configuration.timeSeriesCharts).forEach(function (chart) {
        var container = nepExplorer.containers.timeSeries.append('div').classed("wc-subcomponent wc-subcomponent--chart wc-subcomponent--time-series-chart wc-subcomponent--".concat(chart), true);
        ...
        nepExplorer.containers["".concat(chart, "Header")] = nepExplorer.containers["".concat(chart, "Container")].append('div').classed("wc-header wc-header--".concat(chart), true).text(mergedSettings.title); //.text(syncedSettings.title);

        nepExplorer.containers[chart] = container.append('div').classed("wc-chart-container wc-chart-container--".concat(chart), true);
        var timeSeries = webcharts.createChart(nepExplorer.containers[chart].node(), mergedSettings //syncedSettings
        );
