(function (global, factory) {
    ...
    function styles() {
      ...
      \----------------------------------------------------------------------------*****/
      '.wc-component--kdigo {', '}', '.wc-subcomponent--kdigo-scatter-plot {', '    position: relative;', '}', '.wc-subcomponent--kdigo-scatter-plot circle.wc-hover-mark,', '.wc-subcomponent--kdigo-scatter-plot circle.wc-data-mark {', '    cursor: pointer;', '}', '.wc-subcomponent--kdigo-scatter-plot circle.wc-data-mark {', '    stroke: black;', '    fill-opacity: 1;', '    stroke-width: 1;', '}', '.wc-subcomponent--kdigo-scatter-plot circle.wc-data-mark:hover,', '.wc-subcomponent--kdigo-scatter-plot circle.wc-data-mark.wc-highlighted,', '.wc-subcomponent--kdigo-scatter-plot circle.wc-data-mark.wc-selected {', '    stroke-width: 3;', '}', '.kdigo-stage {', '    stroke: black;', '}', '.wc-subcomponent--kdigo-scatter-plot .tick line {', '    stroke-opacity: .4;', '}', '.wc-subcomponent--kdigo-scatter-plot .legend--kdigo {', '    height: 20px;', '    margin: 8px 0px;', '}', '.wc-subcomponent__legend {', '    position: absolute;', //'    top: 14;',
      //'    bottom: 0;',
      '    left: 80%;', '    width: 100%;', '}', '.wc-subcomponent__legend .wc-chart {', '    width: 100%;', '}', '.wc-subcomponent__legend .interactivity {', '    display: none;', '}', '.wc-subcomponent__legend table {', '    border-collapse: collapse;', '}', '.wc-subcomponent__legend thead {', '    border-top: 2px solid #999;', '    border-bottom: 2px solid #999;', '}', '.wc-subcomponent__legend thead tr {', '    padding: 0.1em;', '}', '.wc-subcomponent__legend tbody {', '    border-bottom: 2px solid #999;', '}', '.wc-subcomponent__legend tbody {', '    border-bottom: 2px solid #999;', '}', '.wc-subcomponent__legend tbody tr {', '    padding: 0.1em;', '}', '.wc-subcomponent__legend tbody td {', '    text-align: center;', '    font-size: 0.9em;', '    padding: 0 0.5em 0 0.5em;', '}',
      /****---------------------------------------------------------------------------------\
        section: participant
      \---------------------------------------------------------------------------------****/
    ...
    function defineParticipantLevelData(_ref) {
      ...
      var participantLevel = d3.nest().key(function (d) {
        ...
        data.forEach(function (d) {
          ...
          d.fchg = baseline && baseline.result > 0 ? Math.round(d.result / baseline.result * 100) / 100 : null;
          d.pchg = baseline && baseline.result > 0 ? Math.round((d.result / baseline.result - 1) * 100 * 100) / 100 : null;
          d.xuln = d.result > 0 && d.uln > 0 ? Math.round(d.result / d.uln * 100) / 100 : null;
        });
        var datum = {
          data: data
        };
          ...
          switch (d.label) {
            case 'X-axis':
              updateOptionText.call(chart, options, function (text) {
                ...
                return "".concat(measure).concat(result.replace('chg', '').replace('f', ' Fold').replace('p', ' Percent'), " Change");
              });
    ...
    function addVoronoiDiagramContainer() {
      this.voronoiDiagramContainer = this.svg.append('g').classed('wc-voronoi-diagram', true).append('defs');
    }
    ...
    function onPreprocess() {
      ...
      this.config.y.domain[0] = this.config.x.type === 'log' ? d3.min(this.raw_data, function (d) {
        return d[_this.config.y.column];
      }) : 0; // setXLabel

      this.config.x.result = this.config.x.column.split('_').pop();
      this.config.x.measure = this.config.measure_values[this.config.x.column.replace("_".concat(this.config.x.result), '')];
      this.config.x.label = "".concat(this.config.x.measure).concat(this.config.x.result.replace('chg', '').replace('f', ' Fold').replace('p', ' Percent'), " Change"); // setYLabel

      this.config.y.result = this.config.y.column.split('_').pop();
      this.config.y.measure = this.config.measure_values[this.config.y.column.replace("_".concat(this.config.y.result), '')];
      this.config.y.label = "".concat(this.config.y.measure).concat(this.config.y.result.replace('chg', '').replace('f', ' Fold').replace('p', ' Percent'), " Change"); // chooseCriteria
      ...
      this.nepExplorer.containers.kdigoHeader.text(this.config.criteria === this.config.kdigo_criteria ? 'KDIGO Scatter Plot' : 'KDIGO-DC Scatter Plot'); // defineKdigoStage

      this.raw_data.forEach(function (d) {
        var kdigo = _this.config.criteria.slice().sort(function (a, b) {
          return b.x - a.x;
        }).find(function (criterion) {
          return criterion.x <= d[_this.config.x.column] || criterion.y <= d[_this.config.y.column];
        }).label;

        d.kdigo = kdigo ? kdigo.replace(/stage_(\d)/, 'Stage $1 AKI').replace('no_aki', 'No AKI') : '???';
      }); // setLegendLabel

      this.config.legend.label = this.config.color_by && this.config.color_by !== 'None' ? this.config.groups.find(function (group) {
    ...
    function updatePopCount() {
      this.controls.popCount.html("<span class = 'numerator'>".concat(this.filtered_data.length, "</span> of <span class = 'numerator'>").concat(this.nepExplorer.data.participants.length, "</span> participants shown."));
    }
    ...
    function drawVoronoiDiagram() {
      ...
      var voronoiClipPaths = this.voronoiDiagramContainer.selectAll('clipPath').data(voronoiData).enter().append('clipPath').attr({
      ...
      var hoverMarks = mark.groups.filter(function (d) {
        ...
        return _this.y(d.values.y);
      }).attr('r', 50).style('fill', 'lightblue').style('pointer-events', 'all').style('fill-opacity', 0); //this.svg.selectAll('path.voronoi-partition').remove();
    ...
    function defineVisitLevelData() {
      ...
      var visits = studydays.map(function (studyday) {
        ...
        visitObj.kdigo = kdigo ? kdigo.replace(/stage_(\d)/, 'Stage $1 AKI').replace('no_aki', 'No AKI') : '???';
        return visitObj;
      }).sort(function (a, b) {
    ...
    function drawPath() {
      ...
      var visitPath = this.hysteresisPlotContainer.append('path').attr('class', 'participant-visits').datum(this.nepExplorer.data.participant.visits).attr('d', line).attr('stroke', function (d) {
    ...
    function drawPoints() {
      ...
      var chart = this;
      var visitContainers = this.hysteresisPlotContainer.selectAll('g.wc-hysteresis-point-container').data(this.nepExplorer.data.participant.visits).enter().append('g').classed('wc-hysteresis-point-container', true);
      visitContainers.each(function (d) {
        ...
        var width = 5;
        if (d.xIsClamped) visitContainer.append('polygon').classed('wc-hysteresis-point--clamped wc-hysteresis-point--clamped--x', true).attr({
          points: [[x, y], [x, y], [x, y]],
          fill: color
        }).transition().delay(2000).duration(200).attr({
          points: [[x, y - width], [x - width, y], [x, y + width]]
        });
        if (d.yIsClamped) visitContainer.append('polygon').classed('wc-hysteresis-point--clamped wc-hysteresis-point--clamped--y', true).attr({
          points: [[x, y], [x, y], [x, y]],
          fill: color
        }).transition().delay(2000).duration(200).attr({
