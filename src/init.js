...
export default function init(data) {
    ...
    const chart2measures = d3.merge(participant.values
        .filter(d => [this.settings.synced.measure_values.creat, this.settings.synced.measure_values.cystatc].includes(d.key))
        .map(d => d.values.data));
    this.chart2.init(chart2measures);
    const chart3measures = d3.merge(participant.values
        .filter(d => (
            [
